...
def send_webhook(job_card_name):
	...
	try:
		...
		frappe.log_error(title="Webhook Error", message=frappe.get_traceback())


@frappe.whitelist()
def trigger_test_error():
	frappe.enqueue("quickfix.api.failing_background_job", queue="short")
...
def get_status_chart_data():
	...
	for status in statuses:
		...
		values.append(count)
	data = {"labels": statuses, "datasets": [{"name": "Job Cards", "values": values}]}
...
def payment_webhook():
	...
	if frappe.db.exists("Audit Log", {"action": "payment_received", "document_name": data.get("ref")}):
...
def get_technician_work_summary():
	...
	technicians = frappe.get_all(
		"Technician", filters={"name": ["in", tech_ids]}, fields=["name", "technician_name", "phone"]
	)
...
def transfer_technician(job_card, new_technician):
	frappe.db.set_value("Job Card", job_card, "assigned_technician", new_technician)
	frappe.db.commit()
	return "transferred"
