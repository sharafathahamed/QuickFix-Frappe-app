...
def get_data(filters):
	...
	for tech, d in tech_data.items():
		...
		avg_turnaround = sum(d["turnaround_days"]) / len(d["turnaround_days"]) if d["turnaround_days"] else 0
		row = {
			"technician": tech,
			"total_jobs": total,
			"completed": completed,
...
def get_summary(data):
	...
	return [
		{"label": "Total Jobs", "value": total_jobs, "datatype": "Int"},
		{"label": "Total Revenue", "value": total_revenue, "datatype": "Currency"},
		{"label": "Best Technician", "value": best_tech["technician"], "datatype": "Data"},
	]
