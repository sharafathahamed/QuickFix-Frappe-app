...
class JobCard(Document):
	def validate(self):
		...
		if (
			status not in ["Draft", "Pending Diagnosis", "Awaiting Customer Approval"]
		...
		if self.assigned_technician:
			...
			if specialization != self.device_type:
				frappe.throw(
					...
					f"and it doesnt match for {self.device_type}"
				)
	...
	def before_print(self, settings=None):
		...
		if self.assigned_technician:
			self.technician_name = frappe.db.get_value(
				"Technician", self.assigned_technician, "technician_name"
			)
	...
	def before_submit(self):
		...
		for row in self.parts_used:
			...
			if flt(curr_stck) < flt(row.quantity):
				...
				frappe.throw(
					f"Insufficient Stock for {part_label}. "
					f"Available: {curr_stck}, Required: {row.quantity}"
				)

	def on_submit(self):
		frappe.enqueue("quickfix.utils.send_webhook", queue="short", job_card_name=self.name)
		for row in self.parts_used:
		...
		frappe.publish_realtime("job_ready", {"job": self.name}, user=self.owner)
	...
	def on_cancel(self):
		...
		for row in self.parts_used:
			...
			frappe.db.set_value(
				...
				row.part,
				"stock_qty",
				flt(current_stock) + flt(row.quantity),
			)
