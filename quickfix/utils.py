...
def check_low_stock():
	last_run = frappe.db.get_value(
		"Audit Log", {"action": "low_stock_check", "creation": ["like", f"{today()}%"]}, "name"
	)
	if last_run:
		return
...
def generate_monthly_revenue_report():
	frappe.get_all(
		"Job Card", filters={"status": "Delivered"}, fields=["name", "final_amount", "delivery_date"]
	)
...
def send_webhook(job_card_name, retry_count=0):
	...
	webhook_id = hashlib.md5(f"{doc.name}job_submitted".encode()).hexdigest()
